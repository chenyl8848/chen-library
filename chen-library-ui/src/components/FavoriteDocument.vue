            ...
            <el-table-column prop="title" label="文档" min-width="200">
                <template #default="scope">
                    <router-link target="_blank" :to="{
                        path: '/document',
                        params: { document_id: scope.row.document_uuid || scope.row.document_id },
...
import { favoriteDocumentList } from '../../mock/data'
...
const getFavoriteDocuments = () => {
    loading.value = true
    setTimeout(() => {
        favorites.value = favoriteDocumentList
        total.value = 41
        loading.value = false
    }, 2000);
}
